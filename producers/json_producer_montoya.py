...
def get_kafka_topic() -> str:
    ...
    topic = os.getenv("BANKS_TOPIC", "banks_data")
...
DATA_FILE: pathlib.Path = DATA_FOLDER.joinpath("banks.json")
...
def generate_messages(file_path: pathlib.Path):
    """
    Read from a JSON file and yield the banks data one by one, continuously.

    Args:
        file_path (pathlib.Path): Path to the JSON file.

    Yields:
        dict: A dictionary containing bank JSON data.
    """
    while True:
        try:
            logger.info(f"Opening data file in read mode: {DATA_FILE}")
            with open(DATA_FILE, "r") as json_file:
                logger.info(f"Reading Bank data from file: {DATA_FILE}")
...
def main():
    ...
    try:
        ...
        logger.info(f"Kafka topic '{topic}' is ready for bank data.")
    except Exception as e:
        logger.error(f"Failed to create or verify topic '{topic}': {e}")
        sys.exit(1)

    # Generate and send messages
    logger.info(f"Starting bank message production to topic '{topic}'...")
    try:
